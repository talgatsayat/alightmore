...
public class PDAnalyst {
    ...
    private List<LogFilterRule> copyFilterCriteria(List<LogFilterRule> criteria) {
        return criteria
            .stream()
            .map(this::createDeepClone)
            .collect(Collectors.toList());
    }
    
    private LogFilterRule createDeepClone(LogFilterRule rule) {
        try {
            // Попробуем использовать deepClone если он доступен
            return rule.deepClone();
        } catch (NoSuchMethodError e) {
            // Если deepClone недоступен, создаем новый объект с теми же параметрами
            return createLogFilterRuleCopy(rule);
        }
    }
    
    private LogFilterRule createLogFilterRuleCopy(LogFilterRule original) {
        // Создаем новый LogFilterRule с теми же параметрами
        Set<RuleValue> primaryValuesCopy = new HashSet<>();
        if (original.getPrimaryValues() != null) {
            for (RuleValue value : original.getPrimaryValues()) {
                primaryValuesCopy.add(new RuleValue(value.getFilterType(), value.getOperationType(), value.getKey(), value.getStringValue()));
            }
        }
        
        Set<RuleValue> secondaryValuesCopy = null;
        if (original.getSecondaryValues() != null) {
            secondaryValuesCopy = new HashSet<>();
            for (RuleValue value : original.getSecondaryValues()) {
                secondaryValuesCopy.add(new RuleValue(value.getFilterType(), value.getOperationType(), value.getKey(), value.getStringValue()));
            }
        }
        
        return new LogFilterRuleImpl(original.getChoice(), original.getInclusion(), original.getSection(),
            original.getFilterType(), original.getKey(), primaryValuesCopy, secondaryValuesCopy);
    }
    ...
    private boolean filterAdditive(LogFilterRule filterCriterion) throws Exception {
        ...
        criteria.add(createDeepClone(filterCriterion));
