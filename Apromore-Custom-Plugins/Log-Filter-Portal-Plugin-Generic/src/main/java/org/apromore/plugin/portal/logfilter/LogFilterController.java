...
//    private Map<String, Set<String>> eventualFollowMap = new HashMap<String, Set<String>>();
    ...
    private List<List<LogFilterCriterion>> previousCriteria = new ArrayList<>();
    private List<List<LogFilterCriterion>> redoCriteria = new ArrayList<>();
    ...
    public void setLogFilterService(LogFilterService logFilterService) {
        this.logFilterService = logFilterService;
    }
    
    public void setLogFilterCriterionFactory(LogFilterCriterionFactoryImpl logFilterCriterionFactory) {
        this.logFilterCriterionFactory = logFilterCriterionFactory;
    }
    
    public void initializeWithLog(PortalContext portalContext, XLog log, LogFilterResultListener resultListener) throws IOException {
        try {
            this.log = log;
            ...
            Messagebox.show("Ошибка инициализации фильтра: " + e.getMessage(), "Ошибка", Messagebox.OK, Messagebox.ERROR);
            throw new IOException("Failed to initialize LogFilterController", e);
        }
    }

    public LogFilterController(PortalContext portalContext, XLog log,
    							LogFilterService logFilterService,
    							LogFilterCriterionFactoryImpl logFilterCriterionFactory,
...
//        this.eventualFollowMap = logStats.getEventualFollowMap();
    ...
    private void initialize(PortalContext portalContext, XLog log, String label, 
    	...
    	try {
            ...
            filterSelectorW = (Window) portalContext.getUI().createComponent(getClass().getClassLoader(), "filterCriteria.zul", null, null);
        ...
        filterSelectorW.doModal();
    		...
    		Messagebox.show("Ошибка инициализации интерфейса фильтра: " + e.getMessage(), "Ошибка", Messagebox.OK, Messagebox.ERROR);
    		throw new IOException("Failed to initialize filter interface", e);
    	}
        filterSelectorW.doModal();
    }
    ...
    private void showSaveDialog(APMLog filteredLog) {
        // Simple message box for now - TODO: implement proper save dialog
        Messagebox.show("Функция сохранения отфильтрованного лога будет реализована в следующей версии", "Информация", Messagebox.OK, Messagebox.INFORMATION);
    }
    
    private XLog convertAPMLogToXLog(APMLog apmLog) {
        // This is a simplified conversion - in a real implementation, you would need to properly convert APMLog to XLog
        // For now, we'll return the original log as a placeholder
        return this.log;
    }
...
//            }
    ...
    private void showUndoRedoDialog() {
        Window undoRedoWindow = new Window();
        undoRedoWindow.setTitle("Управление фильтрами");
        undoRedoWindow.setWidth("400px");
        undoRedoWindow.setHeight("300px");
        undoRedoWindow.setClosable(true);
        undoRedoWindow.setPosition("center");
        ...
        // Undo button
        Button undoButton = new Button("Отменить последний фильтр");
        undoButton.setDisabled(!canUndo);
        ...
        // Redo button
        Button redoButton = new Button("Повторить отмененный фильтр");
        redoButton.setDisabled(!canRedo);
        ...
        Button clearAllButton = new Button("Очистить все фильтры");
        ...
        // Save filter button
        Button saveFilterButton = new Button("Сохранить фильтр");
        ...
        vbox.appendChild(undoButton);
        vbox.appendChild(redoButton);
        vbox.appendChild(clearAllButton);
        vbox.appendChild(saveFilterButton);

        undoRedoWindow.appendChild(vbox);
        undoRedoWindow.doModal();
    }

    public void undoLastFilter() {
        if (canUndo && !previousCriteria.isEmpty()) {
            redoCriteria.add(0, new ArrayList<>(criteria));
            criteria = new ArrayList<>(previousCriteria.remove(previousCriteria.size() - 1));
            canUndo = !previousCriteria.isEmpty();
            canRedo = true;
            applyFilters();
        }
    }
    ...
    public void redoLastFilter() {
        if (canRedo && !redoCriteria.isEmpty()) {
            previousCriteria.add(new ArrayList<>(criteria));
            criteria = new ArrayList<>(redoCriteria.remove(0));
            canUndo = true;
            canRedo = !redoCriteria.isEmpty();
            applyFilters();
        }
    }
    ...
    public void saveCurrentFilter() {
        // Simple message box for now - TODO: implement proper dialog
        Messagebox.show("Функция сохранения фильтров будет реализована в следующей версии", "Информация", Messagebox.OK, Messagebox.INFORMATION);
    }
    ...
    private void applyFilters() {
        ...
        if (output.getTraces().isEmpty()) {
            ...
            Messagebox.show("Фильтры применены успешно. Отфильтровано: " + output.getTraces().size() + " трасс", "Информация", Messagebox.OK, Messagebox.INFORMATION);
        }
        } catch (Exception e) {
    ...
    // Public methods for FilterManagementPanel
    ...
    public List<LogFilterCriterion> getCriteria() {
        return criteria;
    }
